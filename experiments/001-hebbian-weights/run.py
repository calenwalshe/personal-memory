"""
Experiment: 001-hebbian-weights
Hypothesis: Updating relation weights based on entity co-activation across sessions will produce
more semantically coherent communities than static co-occurrence weights.

Hebbian rule: "neurons that fire together, wire together"
  Δw = η × coactivation_count   (for pairs above noise threshold)

Two-phase:
  1. Count how many sessions each entity pair co-appeared in (from atoms.db)
  2. Add Δw to existing edge weights in experimental graph.db
  3. Rebuild communities on updated weights
  4. Write metrics comparing before/after community structure

Mode: official
Run with: vault lab run hebbian-weights
"""
import sys
import json
import sqlite3
...
EXP_DIR = Path("/home/agent/memory/vault/experiments/001-hebbian-weights")
GRAPH_DB = EXP_DIR / "graph.db"
ATOMS_DB = VAULT / "atoms.db"

ETA = 0.3               # learning rate — modest to avoid overwhelming co-occurrence baseline
MIN_COACTIVATIONS = 2   # noise filter: ignore pairs that co-appeared in only 1 session
MAX_DELTA = 3.0         # cap per-pair Δw to prevent runaway amplification
