...
- **Hebbian decay** (scripts/hebbian.py) — live per-session Hebbian plasticity on `related_to` edges:
  - `ETA = 0.3` — per-session weight increment for co-appearing entity pairs
  - `MAX_WEIGHT = 20.0` — soft ceiling preserving gradient spread
  - `DECAY = 0.03` — multiplicative decay for non-firing edges: `w *= (1 - 0.03)`. Half-life ~23 days at 1 session/day. Derived from steady-state formula `w* = p·η / (d·(1-p))`
  - `MIN_WEIGHT = 0.01` — prune threshold: edges below this are deleted, not left as floating-point noise
  - Decay runs AFTER increment in the same `vault chunk` call: `update_from_atoms()` → `apply_decay(fired_entity_ids)`
  - Only edges with `description='hebbian'` are decayed — co-occurrence baseline and typed semantic edges are untouched
  - `vault graph stats` shows: **Stale edges (unfired 30d): N**
